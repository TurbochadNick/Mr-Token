...
function Dashboard({
  ...
  return (
    <div className="dashboard">
      ...
      {/* Lead with REAL transcript-derived numbers when the backend has run; the
          char-counted estimate is only a fallback (it disagrees with real by ~100x
      ...
      {data.accurate.available ? (
        ...
        <section className="dashboardMetrics" aria-label="Estimated (backend not run yet)">
          <MetricCard icon="TK" label="Estimated Tokens" value={formatNumber(data.summary.totalEstimatedTokens)} helper="Char-based estimate — run the backend for real counts." />
...
function AccurateUsage({ accurate }: { accurate: Accurate }) {
  return (
    <section className="section">
      <div className="sectionHeader dashboardSectionHeader">
        <div>
      ...
      {accurate.available ? (
        <>
          <div className="dashboardMetrics" aria-label="Token breakdown from transcripts">
            <MetricCard icon="IN" label="Input" value={formatNumber(accurate.inputTokens)} helper="Fresh (non-cached) input tokens." />
            <MetricCard icon="OU" label="Output" value={formatNumber(accurate.outputTokens)} helper="Tokens generated by the model." />
            <MetricCard icon="CR" label="Cache Read" value={formatNumber(accurate.cacheReadTokens)} helper="Context re-served from cache." />
            <MetricCard icon="CW" label="Cache Write" value={formatNumber(accurate.cacheWriteTokens)} helper="New context written into the cache." />
          ...
          <p>Billing evidence: API {accurate.apiBillingSessions}; subscription {accurate.subscriptionBillingSessions}; UNKNOWN {accurate.unknownBillingSessions}. Cumulative token totals: provider-reported {accurate.providerReportedTotals}; computed from documented disjoint components {accurate.computedTotals}; UNKNOWN {accurate.unknownTotals}. These routes are not combined.</p>
        </>
      ) : (
        <EmptyState
          title="Accurate token data not available yet."
          body="The mrtoken-transcript backend (or its Stop hook) has not populated this database, so the numbers above are estimates. Once it runs, real token counts and cache activity appear here."
