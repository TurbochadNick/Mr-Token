...
| Bind `confirm_disposable` to the calling session | `confirm-disposable-session-binding.md` | Codex / Claude / Fable | **ready** — **pre-6.8 blocker** | no-arg call in a multi-session project can record the confirmation under the OTHER session's id (MCP server has no session env — verified); fix = refuse-on-ambiguity, fail-closed; regression test for the two-pane case |
