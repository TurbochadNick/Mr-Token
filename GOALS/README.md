...
| Explicit disposability channel (`disposable_confirmed`) | `disposable-confirmed-channel.md` | **Claude** (kept in-house — not worth scarce Fable time) | **done** (PR #20) | `confirm_disposable` MCP tool records a session-scoped, metadata-only confirmation (call-index + ts); `intervention_for_session` merges a FRESH one as `disposable_confirmed` → escalation unlocked; stale by CONFIRM_TTL_CALLS(10)/MIN(30) → back to proxy tell-only; skill doc + 92 tests green. **The explicit path to 6.8 L3 now exists end-to-end.** |
