...
| Explicit disposability channel (`disposable_confirmed`) | `disposable-confirmed-channel.md` | Codex / Fable | **ready** (PR #19 merged, reviewed) | agent can confirm droppability via toolbox; fresh confirmation → ask/do escalation works end-to-end; stale/absent → tell-only; tests green. **The path to 6.8 L3 — the proxy is a dead end, so explicit confirmation is the only way to unlock auto-drop.** |
