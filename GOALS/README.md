...
| Compaction gate — phase 3 (remaining-runway gate) | `compaction-gate-phase3.md` | Codex / Fable | **ready** (after phase 2) | `evaluate()` suppresses the reset nudge when near-done; drop requires disposable AND runway; warn-only; `test-backend.sh` green. **Completes the gate → unblocks 6.8 L3.** |
